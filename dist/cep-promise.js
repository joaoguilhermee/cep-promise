(function(global, factory) {
  ...
  fetch =
    fetch && Object.prototype.hasOwnProperty.call(fetch, "default")
      ? fetch["default"]
      : fetch;
  ...
  function parseSuccessXML(xmlString) {
    try {
      ...
      var returnStatement =
        (_xmlString$replace$ma = xmlString
          .replace(/\r?\n|\r/g, "")
          .match(/<return>(.*)<\/return>/)[0]) !== null &&
        _xmlString$replace$ma !== void 0
          ? _xmlString$replace$ma
          : "";
  ...
  function parseAndextractErrorMessage(xmlString) {
    try {
      ...
      var returnStatement =
        (_xmlString$match$ = xmlString.match(
          /<faultstring>(.*)<\/faultstring>/
        )[0]) !== null && _xmlString$match$ !== void 0
          ? _xmlString$match$
          : "";
      var cleanReturnStatement = returnStatement
        .replace("<faultstring>", "")
  ...
  function fetchViaCepService(cepWithLeftPad, configurations) {
    ...
    return fetch(url, options)
      .then(analyzeAndParseResponse$1)
      .then(checkForViaCepError)
      .then(extractCepValuesFromResponse)
      ["catch"](throwApplicationError$1);
  }
  ...
  function fetchWideNetService(cepWithLeftPad, configurations) {
    var url = "https://cep.widenet.host/busca-cep/api/cep/".concat(
      cepWithLeftPad,
      ".json"
    );
    var options = {
    ...
    return fetch(url, options)
      .then(analyzeAndParseResponse$2)
      .then(checkForWideNetError)
      .then(extractCepValuesFromResponse$1)
      ["catch"](throwApplicationError$2);
  }
  ...
  function fetchBrasilAPIService(cepWithLeftPad, configurations) {
    ...
    return fetch(url, options)
      .then(parseResponse)
      .then(extractCepValuesFromResponse$2)
      ["catch"](throwApplicationError$3);
  }
  ...
  function cepPromise(cepRawValue) {
    ...
    return Promise$1.resolve(cepRawValue)
            ...
            errors: [
              {
                message: 'O provider "'
                  .concat(
                    provider,
                    '" \xE9 inv\xE1lido. Os providers dispon\xEDveis s\xE3o: ["'
                  )
                  .concat(availableProviders.join('", "'), '"].'),
                service: "providers_validation"
              }
            ]
          });
  ...
  function validateInputLength(cepWithLeftPad) {
    ...
    throw new CepPromiseError({
      ...
      errors: [
        {
          message: "CEP informado possui mais do que ".concat(
            CEP_SIZE,
            " caracteres."
          ),
          service: "cep_validation"
  ...
  function fetchCepFromServices(cepWithLeftPad, configurations) {
    ...
    if (configurations.providers.length === 0) {
      return Promise$1.any(
        Object.values(providersServices).map(function(provider) {
    ...
    return Promise$1.any(
      configurations.providers.map(function(provider) {
